 ...
 */
@CheckResult
fun MotionLayout.transitionChanged(capacity: Int = 0): ReceiveChannel<MotionLayoutTransition> = cancelableChannel(capacity) {
  val listener = object : MotionLayout.TransitionListener {
    override fun onTransitionChange(motionLayout: MotionLayout, startId: Int, endId: Int, progress: Float) {
      safeOffer(MotionLayoutTransitionChanged(
          motionLayout,
          startId,
          endId,
          progress
    ...
    override fun onTransitionCompleted(motionLayout: MotionLayout, currentId: Int) {
      safeOffer(MotionLayoutTransitionCompleted(
          motionLayout,
          currentId
      ))
    }
