 ...
 */
@CheckResult
fun TabLayout.selections(capacity: Int = 0): ReceiveChannel<TabLayout.Tab> = cancelableChannel(capacity) { onAfterClosed ->
 ...
 */
@CheckResult
fun TabLayout.selectionEvents(capacity: Int = 0): ReceiveChannel<TabLayoutSelectionEvent> = cancelableChannel(capacity) { onAfterClosed ->
  val listener = object : TabLayout.OnTabSelectedListener {
    override fun onTabReselected(tab: TabLayout.Tab) {
